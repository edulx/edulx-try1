      ...
      <div className="flex h-full justify-center items-center px-4">
        <div className="rounded-xl shadow-md bg-white dark:bg-darkgray p-6 w-full md:w-96 border-none">
          <div className="flex flex-col gap-2 p-0 w-full">
            ...
            <p className="text-sm text-center text-dark my-3">Sign In to Edulx</p>
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <button type="submit" className="bg-primary text-black py-2 rounded">Login</button>
              {message && <p className="text-center text-sm mt-2">{message}</p>}
            </form>
            <div className="flex gap-2 text-sm justify-center mt-4">
              <p>New to Edulx?</p>
              <Link to="/auth/register" className="text-primary font-medium">Create an account</Link>
            </div>
          </div>
        </div>
      </div>
